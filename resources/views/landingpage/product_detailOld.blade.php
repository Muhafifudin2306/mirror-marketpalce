    ...
    <div class="container product-card">
      ...
      <div class="row g-5">
        ...
        <div class="col-lg-5">
          ...
          @if($product->discount_percent||$product->discount_fix)
            <h4 style="font-family:'Poppins';font-size:2rem;color:#fc2865;">Rp {{ number_format($final,0,',','.') }}</h4>
            <p style="font-family:'Poppins';color:#c3c3c3;"><del>Rp {{ number_format($product->price,0,',','.') }}</del></p>
          @else
          ...
          @php
            ...
            $finalBase = $base;
            if($product->discount_percent) {
                $finalBase -= $base * $product->discount_percent / 100;
            }
            if($product->discount_fix) {
                $finalBase -= $product->discount_fix;
            }
          ...
          <form id="orderForm" method="POST" action="{{ route('orders.store') }}" enctype="multipart/form-data">
            ...
            @if($product->discount_percent)
              <input type="hidden" name="discount_percent" value="{{ $product->discount_percent }}">
            @elseif($product->discount_fix)
              <input type="hidden" name="discount_fix"    value="{{ $product->discount_fix }}">
            @endif
