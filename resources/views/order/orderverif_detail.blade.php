...
<div class="container flex-grow-1 container-p-y">
    ...
    <div class="d-flex justify-content-between align-items-center mb-4">
        <h3 class="m-0">Detail Verifikasi Pesanan</h1>
        <a href="{{ route('orders.orderVerif') }}" class="btn btn-outline-secondary">
    ...
    {{-- Form Verifikasi --}}
    <form id="confirm-verif-form" method="POST" action="{{ route('orders.confirmOrderVerif', $order->id) }}">
        @csrf
    </form>
    <div class="d-flex justify-content-start gap-2 mb-3">
        @if($order->status_pengerjaan === 'produksi')
            <button class="btn btn-secondary" disabled>
                <i class="bx bx-check-circle me-1"></i> Sedang Produksi
            </button>
        @else
            <button id="btn-verify" class="btn btn-success">
                <i class="bx bx-check-circle me-1"></i> Lanjutkan Produksi
            </button>
        @endif
    </div>
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        document.getElementById('btn-verify').addEventListener('click', () => {
            Notiflix.Confirm.show(
                'Konfirmasi Pesanan',
                'Yakin melanjutkan order ini ke produksi?',
                'Ya',
                'Batal',
