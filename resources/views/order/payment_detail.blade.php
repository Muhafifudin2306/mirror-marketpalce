...
@section('content')
    <div class="container flex-grow-1 container-p-y">
        <div class="d-flex justify-content-between align-items-center mb-4">
            <h3 class="m-0">Detail Verifikasi Pembayaran</h1>
                <a href="{{ route('orders.payment') }}" class="btn btn-outline-secondary">
            ...
            <div class="card-body">
                ...
                @if ($order->status_pembayaran == 0)
                    ...
                    <form id="confirm-payment-form" method="POST"
                        action="{{ route('orders.confirmPayment', $order->id) }}" enctype="multipart/form-data">
                        @csrf
                ...
                @elseif($order->status_pembayaran == 1)
                    <form id="finish-payment-form" method="POST"
                        action="{{ route('orders.finishPelunasanPayment', $order->id) }}" enctype="multipart/form-data">
                        @csrf
                        {{-- PARTIAL --}}
                        @php
                ...
                <table class="table table-bordered">
                    ...
                    <tbody>
                        ...
                        @elseif ($order->diskon_persen)
                            ...
                            <td>Rp{{ number_format(($order->diskon_persen / 100) * ($order->subtotal - $order->ongkir)) }}
                            </td>
                            <td>-
                                Rp{{ number_format(($order->diskon_persen / 100) * ($order->subtotal - $order->ongkir)) }}
                            </td>
                            </tr>
                        @endif
                    </tbody>
                        ...
                        <tr>
                            ...
                            @if ($order->potongan_rp)
                                <th class="fw-bold fs-5">
                                    Rp{{ number_format($order->subtotal - $order->potongan_rp, 0, ',', '.') }}</th>
                            @elseif ($order->diskon_persen)
                                <th class="fw-bold fs-5">
                                    Rp{{ number_format($order->subtotal - ($order->diskon_persen / 100) * ($order->subtotal - $order->ongkir), 0, ',', '.') }}
                                </th>
                            @else
    ...
    <script>
        document.addEventListener('DOMContentLoaded', () => {
            ...
            btnLunas.addEventListener('click', () => {
                const metodePaid = document.getElementById('metode_transaksi_paid').value;
                const buktiLunas = document.getElementById('bukti_lunas').value;
