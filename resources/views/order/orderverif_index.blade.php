    ...
    <div class="card" style="overflow-x: auto;">
        <div class="card-header border-bottom pb-2 mb-2">
            <div class="d-flex justify-content-between align-items-center mb-4">
                <h4 class="m-0">Verifikasi Pesanan</h4>
            </div>
        </div>
        <div class="card-body">
            <x-filter-form :action="route('orders.orderVerif')" :reset-url="route('orders.orderVerif')" :show-spk="true" :show-invoice="false" :show-payment-method="false"
                :show-status-pemesanan="true" form-id="filterForm" />
            <div class="mt-5">
                    ...
                    <tbody>
                        @forelse($ar_order as $i => $order)
                            <tr>
                                ...
                                <td>
                                    <span
                                        class="badge {{ $order->status_pengerjaan == 'verif_pesanan' ? 'bg-label-primary' : 'bg-label-success' }}">
                                        {{ $order->status_pengerjaan == 'verif_pesanan' ? 'Belum Produksi' : 'Sedang Produksi' }}
                                    </span>
                                </td>
                                @can('orderVerif-manipulation')
                                <td>
                                        ...
                                        <ul class="dropdown-menu dropdown-menu-end">
                                            <li>
                                                <a class="dropdown-item"
                                                    href="{{ route('orders.orderVerifShow', $order->id) }}">
